...
<template>
  <div class="">
    <VanNavBar>
      <template #left>
        <div class="flex items-center gap-8 border-none!" @click="router.push('/talent/city')">
    ...
    <div class="latest position-sticky top-0 z-10 w-full flex items-center justify-between bg-[color:var(--van-background-2)] pa-16">
      ...
      <div class="flex gap-8 filter">
        <VanButton icon="location-o" icon-position="right" size="small" plain class="border-none!" @click="router.push('/talent/position-type')">
          前端工程师
        </VanButton>
        <VanButton icon="location-o" icon-position="right" size="small" plain class="border-none!" @click="router.push('/talent/filter')">
          筛选
        </VanButton>
      </div>
    </div>
